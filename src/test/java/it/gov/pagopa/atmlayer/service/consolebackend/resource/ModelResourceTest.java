...
public class ModelResourceTest {
    ...
    void testGetAllBpmn() {
        ...
        when(modelService.findByUserId(any())).thenReturn(Uni.createFrom().item(userProfileDto));
        when(modelService.getBpmnFiltered(0, 1, "functionType", "modelVersion", "CREATED", "acquirerId", "fileName")).thenReturn(Uni.createFrom().item(response));
        given()
                .header(header)
